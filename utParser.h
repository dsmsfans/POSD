...
TEST_F(ParserTest, MatchingSuccess) {
  ...
  // EXPECT_EQ("X", et->right->left->left->term->symbol());
  // EXPECT_EQ("2", et->right->left->right->term->symbol());
  
  // EXPECT_EQ("Y", et->right->right->left->term->symbol());
  // EXPECT_EQ("s(s(X))", et->right->right->right->term->symbol());

  // for(int i = 0;i < terms.size();i++)
  // {
  //   std::cout << terms[i]->symbol() << std::endl;
  // }
  
  // Struct *st = dynamic_cast<Struct *>(terms[5]);
  // st=dynamic_cast<Struct *>(st->args(0));
  // terms[2]->match(*(st->args(0)));
  EXPECT_TRUE(et->evaluate());
